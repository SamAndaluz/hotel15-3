...
class SaleOrderLine(models.Model):
    ...
    def _get_display_price(self, product):
        for rec in self:
            if rec.booking_slot_id:
                currency_id = rec.booking_slot_id.with_context(pricelist=self.order_id.pricelist_id.id).currency_id
                price = rec.booking_slot_id.with_context(pricelist=self.order_id.pricelist_id.id).price
